...
class IdentifierEngine:
    ...
    Example : "sub-15464521_image.png" and "sub-25484441_image.png" would both have the same identifier "_image.png" if
        using a regular expression that is keeping everything after the first "_" like "/_.*$/".
    ...
    def get_identifier(self, path, prefix_file_with_parent_directory=False):
        ...
        If no match is found, the entire file/directory name is used instead since we prefer to have identifier that
        are unique but can still be used in the output vs an empty identifier.
        ...
        Extraction method with the default regular expression :
            ...
            Directories = "^.*-" : Keep everything until the first "-". This way, subject directories like "sub-012012"
                are all aggregated under "sub-" while directory names without "-" are kept entirely.
        ...
        prefix_file_with_parent_directory : bool, default = False
            ...
            This is used to discriminate files that have the same name but are located under different subdirectories
            when filename are expected to be found at multiple places for each subject/configuration.

        Returns
        -------
        identifier : string
            The path's extracted identifier. Will be used to aggregate data on files/directories with the same
            identifier across the repeating file structure.
        """
        path = Path(path)
